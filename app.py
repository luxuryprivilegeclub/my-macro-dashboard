...
st.markdown("""<div style="background: linear-gradient(90deg, #0f2027 0%, #203a43 50%, #2c5364 100%); padding: 20px; border-radius: 15px; text-align: center; color: white; margin-bottom: 25px;">
    ...
    <p style="margin: 5px 0 0 0; opacity: 0.8; font-size: 14px;">Smart Money Concept & Institutional Analysis | 2026 Version</p>
...
def draw_meter(col, name, latest, prev, info_next, msg, dxy_b, gold_b, color, bg, height=250):
    with col:
        st.markdown(f"<p style='text-align: center; font-weight: bold; margin-bottom: -40px; color: #000;'>{name}</p>", unsafe_allow_html=True)
        fig = go.Figure(go.Indicator(mode="gauge+number", value=latest, gauge={'bar': {'color': color}}))
        fig.update_layout(height=height, margin=dict(l=30, r=30, t=50, b=0))
        st.plotly_chart(fig, use_container_width=True)
        st.markdown(f"""<div style="background:{bg}; border:1px solid {color}44; padding:10px; border-radius:10px; text-align:center; margin:-30px auto 30px auto; max-width: 280px;">
            <p style="margin:0; font-size:12px; font-weight:500;">{msg}</p>
            <div style="margin-top:5px;">
                <span style="background:#1e3c72; color:white; padding:2px 8px; border-radius:4px; font-size:10px; font-weight:bold;">{dxy_b}</span>
                <span style="background:#d4af37; color:black; padding:2px 8px; border-radius:4px; font-size:10px; font-weight:bold;">{gold_b}</span>
            </div>
            <p style="margin-top:8px; color:#000; font-size:11px; font-weight:bold;">📅 Next: {info_next}</p>
        </div>""", unsafe_allow_html=True)

# --- SECTION 1: PRIMARY MACRO (2 Columns) ---
st.markdown("<h2 style='text-align: center; color: #2c3e50;'>📌 Key Economic Indicators</h2>", unsafe_allow_html=True)
cols1 = st.columns(2)
main_ind = {
...
for i, (name, info) in enumerate(main_ind.items()):
    ...
    elif 'NFP' in name: msg = "Jobs data strong hai." if latest > prev else "Jobs weak hai."
    draw_meter(cols1[i%2], name, latest, prev, info['next'], msg, d_b, g_b, color, bg, 260)
...
for i, (name, info) in enumerate(liq_ind.items()):
    ...
    draw_meter(cols2[i], name, latest, prev, info['next'], "Macro trend check.", "DXY: TREND", "GOLD: TREND", color, "#fafafa", 200)
...
# Fed Target Meter (4th in Section 3)
cpi_data = fred.get_series('CPIAUCSL')
curr_inf = ((cpi_data.iloc[-1] - cpi_data.iloc[-13]) / cpi_data.iloc[-13]) * 100
draw_meter(cols3[3], "Inf. vs 2% Target", curr_inf, 2.0, "Feb 13", f"Gap: {curr_inf-2:.2f}%", "Hawkish Fed", "Gold Pressure", "red", "#fafafa", 200)

# --- SECTION 4: COT REPORT (DETAILED) ---
...
with col_l:
    fig_cot = go.Figure(go.Indicator(mode="gauge+number", value=sentiment, title={'text': "Bullish Index %"}, gauge={'bar': {'color': "#d4af37"}}))
...
with col_r:
    st.markdown(f"""<div style="background:white; border:2px solid #d4af37; padding:20px; border-radius:15px; margin-top:20px;">
        ...
        <hr><div style="display:flex; justify-content:space-around; text-align:center;">
            <div><p style="margin:0; font-size:10px;">Longs</p><b>{rep['longs']:,}</b></div>
            <div><p style="margin:0; font-size:10px;">Shorts</p><b>{rep['shorts']:,}</b></div>
            <div><p style="margin:0; font-size:10px;">Net</p><b>{rep['longs']-rep['shorts']:,}</b></div>
